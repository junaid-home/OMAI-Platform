...
test("resolves directory autocomplete from the current browser root", async () => {
  ...
  const browser = {
    defaultRoot: () => Promise.resolve(undefined),
    list: () => Promise.resolve([]),
    search: (directory: string) => {
      directories.push(directory)
      return Promise.resolve([])
    },
  } satisfies Parameters<typeof createDirectorySearch>[0]["browser"]
  let base = "/repo"
  const search = createDirectorySearch({ browser, home: () => "/home/luke", base: () => base })
