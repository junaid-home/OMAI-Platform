...
import { Dialog, DialogBody, DialogFooter, DialogHeader, DialogTitle } from "@omai/ui/v2/dialog-v2"
import { ButtonV2 } from "@omai/ui/v2/button-v2"
import { TextInputV2 } from "@omai/ui/v2/text-input-v2"
...
import { DividerV2 } from "@omai/ui/v2/divider-v2"
import { omaiWorkspaceBrowser } from "@/omai/workspace-browser"
...
export function DialogSelectDirectoryV2(props: DialogSelectDirectoryV2Props) {
  ...
  const [fallbackPath] = createResource(
    ...
    () => omaiWorkspaceBrowser.defaultRoot().catch(() => undefined),
    { initialValue: undefined },
  )
  const home = createMemo(() => sync.data.path.home || fallbackPath() || "")
  const start = createMemo(() => props.start || sync.data.path.home || sync.data.path.directory || fallbackPath())
  const search = createDirectorySearch({ browser: omaiWorkspaceBrowser, home, base: () => root() || start() })
  const [suggestions] = createResource(input, async (value) => {
    ...
    const files = await omaiWorkspaceBrowser
      .search(root(), pickerFileSearchQuery(root(), value, home()), "file", 20)
      .catch(() => [])
    const results = [
      ...directories,
