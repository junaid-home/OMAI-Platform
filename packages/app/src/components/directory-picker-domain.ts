...
import type { OMAIWorkspaceBrowser } from "@/omai/workspace-browser"
...
export function createDirectorySearch(args: {
  ...
  return async (filter: string) => {
    ...
    if (!pathInput) {
      const results = await args.browser
        .search(input.directory, query, "directory", 50)
        .catch(() => [])
        .catch(() => [])
      if (!active()) return []
