import { withAlpha } from "@omai/ui/theme/color"
import { useTheme } from "@omai/ui/theme/context"
import { resolveThemeVariant } from "@omai/ui/theme/resolve"
import { resolveThemeVariantV2 } from "@omai/ui/theme/v2/resolve"
import type { HexColor, ResolvedV2Theme } from "@omai/ui/theme/types"
...
export const Terminal = (props: TerminalProps) => {
  ...
  const pushSize = (cols: number, rows: number) => {
    return omaiClient.terminals.resize(id, cols, rows).catch((err) => {
  ...
  const flushInput = () => {
    ...
    const data = inputEncoder.encode(inputQueue.splice(0).join(""))
    for (let offset = 0; offset < data.length; offset += 64 * 1024) {
      const chunk = data.slice(offset, Math.min(offset + 64 * 1024, data.length))
      inputWrites = inputWrites
        .then(() => omaiClient.terminals.write(id, chunk))
        .catch((error) => {
          debugTerminal("failed to write terminal input", error)
  ...
  const queueInput = (data: string) => {
    ...
    if (inputQueue.reduce((total, value) => total + value.length, 0) >= 16 * 1024) {
      if (inputTimer !== undefined) clearTimeout(inputTimer)
      flushInput()
      return
    }
    if (inputTimer !== undefined) return
  ...
  onMount(() => {
    const run = async () => {
      ...
      const gone = () =>
        resolveOMAIWorkspace(directory)
          .then((workspace) => omaiClient.terminals.list(workspace.id))
          .then((terminals) => !terminals.some((terminal) => terminal.id === id))
          .catch((err) => {
      ...
      const open = async () => {
        ...
        let exited = false
        try {
          for await (const chunk of omaiClient.terminals.watch(id, BigInt(Math.max(seek, 0)), {
            signal: controller.signal,
          })) {
            if (disposed || controller.signal.aborted) return
            const next = Number(chunk.cursor)
            if (!Number.isSafeInteger(next) || next < 0) throw new Error("OMAI returned an invalid terminal cursor")
            cursor = next
            seek = next
            const data = decoder.decode(chunk.data, { stream: !chunk.exited })
            if (data) output?.push(data)
            if (!chunk.exited) continue
            exited = true
            local.onExit?.()
            return
          }
        ...
        if (!disposed && !exited) retry(new Error("OMAI terminal stream ended without an exit event"))
      }
