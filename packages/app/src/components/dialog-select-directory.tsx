...
import { List } from "@omai/ui/list"
import type { ListRef } from "@omai/ui/list"
...
export function DialogSelectDirectory(props: DialogSelectDirectoryProps) {
  ...
  const { sync, ...serverCtx } = global.ensureServerCtx(props.server)
  ...
  const [fallbackPath] = createResource(
    ...
    async () => {
      return omaiWorkspaceBrowser.defaultRoot().catch(() => undefined)
    },
    { initialValue: undefined },
  )

  const home = createMemo(() => sync.data.path.home || fallbackPath() || "")
  const start = createMemo(() => sync.data.path.home || sync.data.path.directory || fallbackPath())

  const directories = createDirectorySearch({
    browser: omaiWorkspaceBrowser,
    home,
    base: start,
  })
