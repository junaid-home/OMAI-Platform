...
function SessionTabSlot(props: {
  ...
  const tabInfo = createMemo(() => tabs.info[props.id])
  const [rpcSessionsStore] = persisted(
    rpcSessionsPersist(),
    createStore<{ sessions: RpcSessionMeta[] }>({ sessions: [] }),
  )
  const isRpcSession = () => rpcSessionsStore.sessions.some((session) => session.id === props.tab.sessionId)
  const [loadedSession] = createResource(
    () => {
      const ctx = props.serverCtx()
      if (!ctx || isRpcSession()) return null
  ...
  return (
    <div
      ...
      classList={{ hidden: !session() && !missingSession() && !tabInfo()?.title }}
    >
      <TabNavItem
        ref={(el) => {
          ref = el
        ...
        session={session}
        fallbackTitle={tabInfo()?.title ?? (missingSession() ? language.t("session.tab.unknown") : undefined)}
        onTitleChange={(title) => {
          const value = session()
          const ctx = props.serverCtx()
