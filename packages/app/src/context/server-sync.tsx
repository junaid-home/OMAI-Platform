...
import { omaiSessionView } from "@/omai/sessions"
import { listOMAIMCP, toggleOMAIMCP } from "@/omai/mcp"
...
export const loadMcpResourcesQuery = (scope: ServerScope, directory: string, sdk: OpencodeClient) =>
  queryOptions<Record<string, McpResource>>({
    ...
    queryFn: () => {
      // MCP resources become visible only after the Go-owned execution
      // adapter has completed capability discovery. Configuration alone must
      // never be presented as a connected runtime.
      void sdk
...
export const loadLspQuery = (scope: ServerScope, directory: string, sdk: OpencodeClient) =>
  queryOptions({
    ...
    queryFn: async () => {
      ...
      const instances = await omaiClient.lsp.instances(workspace.id)
      return instances.map((instance) => ({
        id: instance.id,
        name: instance.serverId,
        root: directory,
        status: instance.status === "running" ? ("connected" as const) : ("error" as const),
      }))
    },
  })
...
export function createServerSyncContextInner(serverSDK: ServerSDK) {
      ...
      .fetchQuery({
        ...
        queryFn: () =>
          omaiClient.projects
            .resolve(directory)
            .then(async (project) => ({
              data: (await omaiClient.sessions.list(project.id)).map((item) => omaiSessionView(item, project)),
              limit,
              limited: false,
            }))
            .then((x) => {
  ...
  async function bootstrapInstance(directory: string) {
    ...
    const promise = Promise.resolve().then(async () => {
      ...
      if (!cache) return
      const project = omaiProjectView(await omaiClient.projects.resolve(directory))
      setProjects((projects) => {
        const index = projects.findIndex((item) => item.id === project.id)
        if (index === -1) return [...projects, project].sort((a, b) => (a.id < b.id ? -1 : a.id > b.id ? 1 : 0))
  ...
  return {
    ...
    mcp: {
      toggle: async (directory: string, name: string) => {
        const key = directoryKey(directory)
        await toggleOMAIMCP(directory, name)
