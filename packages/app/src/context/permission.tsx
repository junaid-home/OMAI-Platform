...
import { listOMAIPermissionsForRoot, respondOMAIPermission } from "@/omai/interactions"
...
function createServerPermissionState(input: { sdk: ServerSDK; sync: ServerSync }) {
  ...
  const respond: PermissionRespondFn = (request) => {
    ...
    respondOMAIPermission(request).catch(() => {
  ...
  // Go session streams update the shared pending-resource store directly.
  // React to that authoritative state as well as UI-local compatibility events.
  createEffect(() => {
    if (!ready() || meta.disposed) return
    for (const pending of Object.values(input.sync.session.data.permission)) {
      for (const request of pending ?? []) {
        const directory = input.sync.session.lineage.peek(request.sessionID)?.session.directory
  ...
  function enableDirectory(directory: string) {
    ...
    listOMAIPermissionsForRoot(directory)
      .then((permissions) => {
        if (meta.disposed) return
