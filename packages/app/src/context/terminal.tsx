...
function createWorkspaceTerminalSession(root: string, dir: string, scope: ServerScopeValue, legacySessionID?: string) {
  ...
  const update = (pty: Partial<LocalPTY> & { id: string }) => {
    ...
    if (!pty.cols || !pty.rows) return
    omaiClient.terminals.resize(pty.id, pty.cols, pty.rows).catch((error: unknown) => {
      if (previous) {
  ...
  const clone = async (id: string) => {
    ...
    if (!pty) return
    const next = await resolveOMAIWorkspace(root)
      .then((workspace) => omaiClient.terminals.create({ workspaceId: workspace.id }))
      .catch((error: unknown) => {
  ...
  return {
    ...
    async close(id: string) {
      ...
      await omaiClient.terminals.remove(id).catch((error: unknown) => {
  ...
  init: () => {
    ...
    const loadWorkspace = (dir: string, legacySessionID: string | undefined, serverScope: ServerScopeValue) => {
      ...
      const entry = createRoot((dispose) => ({
        value: createWorkspaceTerminalSession(sdk().directory, dir, serverScope, legacySessionID),
        dispose,
      }))
