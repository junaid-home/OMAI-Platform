...
import { getOMAISessionView } from "@/omai/sessions"
...
function merge<T extends { id: string }>(a: readonly T[], b: readonly T[]) {
  ...
  return [...items.values()].sort((x, y) => {
    const xc = (x as { time?: { created?: number } }).time?.created
    const yc = (y as { time?: { created?: number } }).time?.created
    if (xc !== undefined && yc !== undefined) return xc - yc || cmp(x.id, y.id)
...
export function createServerSession(
  ...
  configuration?: { retry?: typeof retry; resolveSession?: (sessionID: string) => Promise<Session> },
) {
  ...
  const resolve = (sessionID: string, options?: { force?: boolean }) => {
    ...
    const request = (configuration?.resolveSession ?? getOMAISessionView)(sessionID)
      .catch(() => {
  ...
  const fetchMessages = async (sessionID: string, limit: number, before?: string, onAttempt?: () => void) => {
    const response = await (configuration?.retry ?? retry)(() => {
      onAttempt?.()
