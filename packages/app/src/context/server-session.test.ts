...
import type { retry } from "@omai/platform-utils/retry"
import type { Message, OpencodeClient, Part, Session } from "@opencode-ai/sdk/v2/client"
import { createServerSession as createProductionServerSession } from "./server-session"
...
function createServerSession(client: OpencodeClient, options?: { retry?: typeof retry }) {
  return createProductionServerSession(client, {
    ...options,
    resolveSession: async (sessionID) => {
      const response = await client.session.get({ sessionID })
      if (!response.data) throw new Error(`Session not found: ${sessionID}`)
