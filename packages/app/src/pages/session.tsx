...
import type { SessionReviewLineComment } from "@omai/session-ui/session-review"
import { ResizeHandle } from "@omai/ui/resize-handle"
import { Select } from "@omai/ui/select"
import { SelectV2 } from "@omai/ui/v2/select-v2"
import { isScrollKeyTarget, scrollKey, scrollKeyOwner } from "@omai/ui/scroll-view"
import { Tabs } from "@omai/ui/tabs"
import { ButtonV2 } from "@omai/ui/v2/button-v2"
import { Icon } from "@omai/ui/icon"
import { BrandLogo } from "@/components/brand-logo"
import { PreviewPanel } from "@/components/preview-panel"
import { createAutoScroll } from "@omai/ui/hooks"
import { previewSelectedLines } from "@omai/session-ui/pierre/selection-bridge"
import { Button } from "@omai/ui/button"
import { showToast } from "@/utils/toast"
import { base64Encode, checksum } from "@omai/platform-utils/encode"
...
import { RpcChatProvider, RpcChatSessionSeeder } from "@/rpc-chat/provider"
import { rpcSessionsPersist, type RpcSessionMeta } from "@/rpc-chat/catalog"
...
import { SessionReviewEmptyChangesV2 } from "@omai/session-ui/v2/session-review-empty-changes-v2"
import { SessionReviewEmptyNoGitV2 } from "@omai/session-ui/v2/session-review-empty-no-git-v2"
import { SessionReviewV2SidebarToggle } from "@omai/session-ui/v2/session-review-v2"
import { ReviewPanelV2 } from "@/pages/session/v2/review-panel-v2"
import { createReviewPanelV2State } from "@/pages/session/v2/review-panel-v2-state"
import { reviewDiffNeedsLoad } from "@/pages/session/v2/review-diff-kinds"
...
export default function Page() {
  ...
  const [rpcSessionsStore] = persisted(
    rpcSessionsPersist(),
    createStore<{ sessions: RpcSessionMeta[] }>({ sessions: [] }),
  )
  const isRpcSession = (id: string) => rpcSessionsStore.sessions.some((session) => session.id === id)
  ...
  const gitKey = createMemo(() => ["omai-git", sdk().directory] as const)
  const gitStatusQuery = createQuery(() => ({
    queryKey: [...gitKey(), "status"] as const,
    queryFn: async () => {
      const workspace = await resolveOMAIWorkspace(sdk().directory)
      return omaiClient.git.status(workspace.id)
    },
    retry: false,
  }))
  const nogit = createMemo(() => gitStatusQuery.isFetched && !gitStatusQuery.data)
  const changesOptions = createMemo<ChangeMode[]>(() => {
    const list: ChangeMode[] = []
    const vcs = gitStatusQuery.data
    if (vcs) list.push("git")
    if (vcs?.branch && vcs.defaultBranch && vcs.branch !== vcs.defaultBranch) {
      list.push("branch")
    }
    list.push("turn")
    return list
  ...
  const vcsKey = createMemo(
    () => [...gitKey(), "diff", gitStatusQuery.data?.branch ?? "", gitStatusQuery.data?.defaultBranch ?? ""] as const,
  )
  const vcsQuery = createQuery(() => {
    const mode = vcsMode()
    const enabled = wantsReview() && gitStatusQuery.data !== undefined
    ...
    return {
      ...
      queryFn: mode
        ? () =>
            resolveOMAIWorkspace(sdk().directory)
              .then((workspace) => omaiClient.git.diffFiles(workspace.id, mode))
  ...
  const refreshVcs = debounce(() => void queryClient.invalidateQueries({ queryKey: gitKey() }), 100)
  const reviewDiffs = () => {
  ...
  const loadReviewDiff = async (file: string, version?: number): Promise<VcsFileDiff | undefined> => {
    ...
    const request = (context: number) =>
      queryClient
        .fetchQuery({
          queryKey: [...vcsKey(), mode, "file", file, context, version] as const,
          staleTime: Number.POSITIVE_INFINITY,
          retry: 2,
          queryFn: async () => {
            const workspace = await resolveOMAIWorkspace(sdk().directory)
            return omaiClient.git.diffFiles(workspace.id, mode, file, context)
          },
        })
        .then((diffs) => diffs.find((diff) => diff.file === file))

    try {
      const bounded = valid(await request(3))
      if (bounded) return bounded
    } catch (error) {
      console.debug("[session-review] failed to load bounded vcs diff", { mode, file, error })
    }
  }
  ...
  const gitMutation = useMutation(() => ({
    mutationFn: () => resolveOMAIWorkspace(sdk().directory).then((workspace) => omaiClient.git.init(workspace.id)),
    onSuccess: () => {
      const project = sync().project
      if (!project) return
      upsert({ ...project, vcs: "git" })
    },
    onError: (err) => {
      showToast({
        variant: "error",
  ...
  const halt = (sessionID: string) =>
    busy(sessionID) ? omaiClient.sessions.cancel({ sessionId: sessionID }).catch(() => {}) : Promise.resolve()
