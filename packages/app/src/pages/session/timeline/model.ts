...
export function createTimelineModel(input: {
  ...
  const sync = useSync()
  const rpcChatPage = useRpcChat() !== undefined
  ...
  const ready = createMemo(() => {
    const id = input.sessionID()
    if (!id) return true
    if (rpcChatPage) return true
    return loadFailed() || isTimelineReady(sync().data.message[id], serverSync().session.history.loading(id))
  })
  ...
  const more = createMemo(() => {
    const id = input.sessionID()
    return !rpcChatPage && id ? sync().session.history.more(id) : false
  })
  const loading = createMemo(() => {
    const id = input.sessionID()
    return !rpcChatPage && id ? sync().session.history.loading(id) : false
  })
