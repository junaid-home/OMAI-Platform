...
import { Accordion } from "@omai/ui/accordion"
import { Button } from "@omai/ui/button"
import { Card } from "@omai/ui/card"
import {
  ContextToolGroup,
  Message,
  MessageDivider,
  ...
  partDefaultOpen,
  type UserActions,
} from "@omai/session-ui/message-part"
import { DiffChanges } from "@omai/ui/diff-changes"
import { FileIcon } from "@omai/ui/file-icon"
import { Icon } from "@omai/ui/icon"
import { IconButton } from "@omai/ui/icon-button"
import { Icon as IconV2 } from "@omai/ui/v2/icon"
import { IconButtonV2 } from "@omai/ui/v2/icon-button-v2"
import { DropdownMenu } from "@omai/ui/dropdown-menu"
import { MenuV2 } from "@omai/ui/v2/menu-v2"
import { Dialog } from "@omai/ui/dialog"
import { DialogFooter, DialogHeader, DialogTitleGroup, DialogV2 } from "@omai/ui/v2/dialog-v2"
import { InlineInput } from "@omai/ui/inline-input"
import { Spinner } from "@omai/ui/spinner"
import { ButtonV2 } from "@omai/ui/v2/button-v2"
import { SessionRetry } from "@omai/session-ui/session-retry"
import { isScrollKeyTarget, scrollKey, scrollKeyOwner, ScrollView } from "@omai/ui/scroll-view"
import { StickyAccordionHeader } from "@omai/ui/sticky-accordion-header"
import { TextField } from "@omai/ui/text-field"
import { TextReveal } from "@omai/ui/text-reveal"
import { TextShimmer } from "@omai/ui/text-shimmer"
import type {
  AssistantMessage,
...
import { getDirectory, getFilename } from "@omai/platform-utils/path"
import { Popover as KobaltePopover } from "@kobalte/core/popover"
import { normalize } from "@omai/session-ui/session-diff"
import { useFileComponent } from "@omai/ui/context/file"
...
export function MessageTimeline(props: {
  ...
  const rpcChatPage = rpcChat !== undefined
  ...
  const shareMutation = useMutation(() => ({
    mutationFn: async (_id: string) => {
      throw new Error("OMAI session sharing is not available")
    },
    onError: (err) => {
  ...
  const titleMutation = useMutation(() => ({
    mutationFn: async (input: { id: string; title: string }) => {
      const meta = rpcChat?.sessionMeta(input.id)
      const updated = await omaiClient.sessions.update(input.id, { title: input.title })
      if (rpcChat && meta)
        rpcChat.rememberSession({
          ...meta,
          title: updated.title,
          updatedUnixMillis: Number(updated.updatedUnixMillis),
        })
      return updated
    },
    onSuccess: (_, input) => {
  ...
  const archiveSession = async (sessionID: string) => {
    ...
    const request = omaiClient.sessions.update(sessionID, { archived: true })
    await request
      .then(() => {
        if (rpcChat?.sessionMeta(sessionID)) rpcChat.forgetSession(sessionID)
        sync().set(
          produce((draft) => {
  ...
  const deleteSession = async (sessionID: string) => {
    ...
    const meta = rpcChat?.sessionMeta(sessionID)
    const result = await omaiClient.sessions.delete(sessionID).catch((err) => {
      showToast({
    ...
    if (meta) rpcChat?.forgetSession(sessionID)
